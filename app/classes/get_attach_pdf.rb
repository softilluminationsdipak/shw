class GetAttachPdf
  ...
  def self.work_order_file(repair_id)
...
#    property = repair.claim.property
    ...
    claim_type =  repair.claim.present? ? repair.claim.claim_type : ''

require 'prawn'
    doc = Prawn::Document.generate("#{Rails.root}/contracts/workorder_#{claim_number}.pdf",:margin => [20, 20, 40, 20]) do |pdf|
      ...
      #pdf.table(title_data,
       #         :cell_style => {:border_width => 0, :size => 12, :padding => [3,3,3,3], :align => :center},
        #        :column_widths => { 0 => 570 })
...
=end
      ...
      pdf.text "<b><u> DO NOT MAIL INVOICES - EMAIL OR FAX ONLY</u></b>",:inline_format => true,:align => :center
