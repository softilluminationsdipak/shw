...
module BackgrounDRb
  ...
  class MasterWorker
    ...
    def receive_data p_data
      ...
      @tokenizer.extract(p_data) do |b_data|
        ...
        if t_data
          case t_data[:type]
          when :do_work then process_work(t_data)
          when :get_status then process_status(t_data)
          when :get_result then process_request(t_data)
          when :start_worker then start_worker_request(t_data)
          when :delete_worker then delete_drb_worker(t_data)
          when :all_worker_status then query_all_worker_status(t_data)
  ...
  class MasterProxy
    ...
    def load_rails_env
      # lazy_load = BDRB_CONFIG[:backgroundrb][:lazy_load].nil? ? true : BDRB_CONFIG[:backgroundrb][:lazy_load].nil?
    ...
    def check_for_ruby_version; return RUBY_VERSION >= "1.9.3"; end
