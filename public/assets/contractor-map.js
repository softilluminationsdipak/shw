...
var homeMarkerIcon = new GIcon(markerIconBase, "/assets/icons/map/home.png");
var knownMarkerIcon = new GIcon(markerIconBase, "/assets/icons/map/known.png");
var unknownMarkerIcon = new GIcon(markerIconBase, "/assets/icons/map/unknown.png");

// Adapted from http://esa.ilmari.googlepages.com/circle.htm
var drawCircleAtPoint = function(center) {
	// Esa 2006
	//calculating km/degree
	var radius = searchRadiusMiles * 1.609344; // Convert miles to km
...
var contractorsTable = new CITable({
	title: new CITitle({ title: 'Contractors (close &rarr; far)' }),
	hideHeader: true,
	selectable: true,
	get: {
		url: '/admin/contractors/async_find_in_radius',
		paramsFn: function() { return { radius: searchRadiusMiles,searchkey: searchKeyValue, lat: customerMarker.getLatLng().lat(), lng: customerMarker.getLatLng().lng() }; }
	},
	cssStyles: { CIFirmWidth: 250, CIFirmHeight: 270 },
	columns: [
		{ property: 'id', renderer: function(id, contractor) {
			var img = new CIElement('img', { src: '/assets/icons/' + (contractor.flagged ? 'red_flag_16' : 'none') + '.png', alt: 'Flagged' });
			return new CIVPanel({
					...
					new CIImageLink({
						...
						Clicked: function() {
							var object = {
								...
								min_claim: contractor.min_claim,
								min_hour: contractor.min_hour
							};
							var form = new CIForm({
								...
								fields: [
									...
									{ label: 'Min/Claim:', property: 'min_claim', name: 'contractor[min_claim]'},
									{ label: 'Min/Hour:', property: 'min_hour', name: 'contractor[min_hour]'}
								],
		...
		{ property: 'id', cssStyles: { 'font-size': '8pt' }, renderer: function(id, record) {
			return '<strong>{company}</strong><br/>{email}<br/>Tel: {phone}<br/>Fax: {fax}'.substitute({
	...
	GotData: function(collection, json) {
		...
		drawCircleAtPoint(customerMarker.getLatLng());
		...
		map.panTo(customerMarker.getLatLng()); 
...
var searchContractors = function() { searchRadiusMiles = searchRadiusField.getValue(); searchKeyValue=searchKeyField.getValue(); contractorsTable.getData(); };
var searchRadiusField = new CIFormField({
	value: searchRadiusMiles,
	label: 'Radius(mil):', size: 2,
	
	EnterPressed: searchContractors
});

var searchKeyField = new CIFormField({
	...
	content: [
		new CIVPanel([
			...
			new CIHPanel({
				...
				content: [
					{
						cssStyles: { CIFirmWidth: 132 },
						content: searchRadiusField
					},
					{
						cssStyles: { CIFirmWidth: 150 },
						content: searchKeyField
					},
